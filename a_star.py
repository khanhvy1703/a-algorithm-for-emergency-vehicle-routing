...
def a_star(grid, start, goal, return_costs=False, return_expanded=False, 
    ...
    """
    Performs the A* pathfinding algorithm on a 2D grid with obstacles, returning
    not only the path and cost table, but also the number and order of node expansions.
    ...
    Notes:
    Tracks the full order of node expansions (not just which nodes
    were visited) so it visualizes the search process step-by-step.
    """
    open_set = []
    heapq.heappush(open_set, (heuristic(start, goal), 0, start, [start]))
    closed_set = set()      # Used for O(1) lookup for already-expanded nodes
    visit_order = []        # Will record (row, col) for each node, in the order expanded
    costs = {start: (0, heuristic(start, goal), heuristic(start, goal))}
    expanded = 0            # Counter for total expanded nodes

    while open_set:
        est_total, cost, node, path = heapq.heappop(open_set)
        if node == goal:
            # Found a path: assemble output tuple based on flags
            result = [path]
        ...
        if node in closed_set:
            continue  # Node already expanded then skip
        closed_set.add(node)
        visit_order.append(node)   # Record the visit/expansion order
        expanded += 1
        for d in DIRS:
    ...
    # If no path was found, return Nones with other requested info
    result = [None]
    if return_costs: result.append(costs)
    if return_expanded: result.append(expanded)
    if return_visited: result.append(visit_order)
...
def plot_map_with_costs(grid, costs, path=None, title="", start=None, goal=None,
    ...
    if visited:
        vx, vy = zip(*visited)
        plt.scatter(vy, vx, c='orange', s=90, marker='s', alpha=0.5, label='Visited')
    ...
    if path:
        px, py = zip(*path)
        plt.plot(py, px, color='red', linewidth=3, marker='o', markersize=8, label='Path')

    # Draw start and goal
    plt.scatter([start[1]], [start[0]], color='green', s=200, marker='*', edgecolors='black', linewidths=1, zorder=2, label='Start')
    plt.scatter([goal[1]], [goal[0]], color='blue', s=200, marker='*', edgecolors='black', linewidths=1, zorder=2, label='Goal')
...
def create_map(prob_block=0.0, max_attempts=1000, start=None, goal=None):
    ...
    for attempt in range(max_attempts):
        ...
        path, costs = a_star(grid, start, goal, return_costs=True, return_visited=False)
        if path:
    ...
    for i, (prob, title) in enumerate(zip(probs, titles)):
        ...
        try:
            ...
            path_astar, costs_astar, astar_expanded, astar_visit_order = a_star(
                grid, start, goal, return_costs=True, return_expanded=True, return_visited=True)
            t1 = time.time()
            astar_time = t1 - t0
            ...
            print(f"A* expanded {astar_expanded} nodes, time: {astar_time:.6f} sec")
            output_lines.append(f"A* expanded {astar_expanded} nodes, time: {astar_time:.6f} sec")
            print()
            output_lines.append("")
            ...
            if path_astar:
                ...
                try:
                    sys.stdout = tempbuf
                    print_path_costs_and_heuristics(path_astar, goal, algo='astar')
                finally:
                    sys.stdout = old_stdout
            ...
            plot_map_with_costs(grid, costs_astar, path_astar, title=title + " (A*)",
                                start=start, goal=goal, filename=img_filename_astar, visited=astar_visit_order)
